...
3. **신선 드라이런** — 이 저장소를 모르는 신선한 에이전트가 **영문 README만 보고** 설치 → `tests/fixtures/messy-project` 검진 → `tests/compare_report.py` 채점 100%, 설치 복붙부터 보고서까지 5분 이내(Claude Code·git 기설치 기준, PowerShell·bash 각 1회).
