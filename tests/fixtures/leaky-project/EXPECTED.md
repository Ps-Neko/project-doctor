...
참고: 이력 주입(setup-leaky-history.ps1) 사본을 채점할 때 SEC-02는 "추가 보고 허용(채점 중립)"으로 처리된다 — 정확성 대조(커밋 해시·값 미출력)는 아래 별도 절의 수동 확인 항목.
...
아래 ID는 보고돼도 오탐이 아니고, 빠져도 감점이 없다:

- SEC-02 — 이력 주입(setup-leaky-history.ps1) **사본**에서만 발견 가능한 항목 (아래 절). 원본 픽스처에는 git이 없어 정상 검진이면 보고되지 않는다.
...
- **오탐 0건** — 정답지·채점 중립에 없는 ID가 보고되면 compare_report.py가 자동으로 미달 처리한다.
  단 이 게이트는 ID 수준이다: 정답 ID와 같은 ID로 합쳐지는 파일 수준 오탐(예: docs/usage.md의
  플레이스홀더를 SEC-01로 보고)은 "오탐 금지 목록" 대비 사람이 본문에서 대조한다.
