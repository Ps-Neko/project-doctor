...
<div class="doc">
  ...
  <p class="machine"><span class="t">기계 판독용 · 변경 금지</span><span class="mono">스킬 버전 · 발견ID: …</span><span class="warn">다음 검진 비교용 — 수정 금지 안내</span></p>
...
- 재검진이면 "지난 검진과 비교" 표는 03 다음에 같은 표 골격으로.
- **기계 판독 줄(`발견ID:` 등 이 버전 보고서 계약에 있는 한 줄들)은 `.machine`의 `.mono`에 텍스트 그대로 포함** — 생략 금지 (이 파일만 남아도 추적 가능해야 한다).
