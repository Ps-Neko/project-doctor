...
- **저장 전 비밀키 자가 점검 (md·HTML·PDF 모두):** md 보고서의 비밀키 점검은 **SKILL.md의 내보내기 전 모델 자가 점검**이 담당한다 — 형식 검증기 `verify_report_format.py`(FORM-11)는 CI·내부 측정 루프용 도구이며 설치본(`~/.claude/skills/project-doctor/tools/`)에 포함되지 않으므로 런타임에 호출하지 않는다. HTML/PDF는 스킬과 함께 설치되는 `tools/verify_html_report.py`(HTML-06)가 §2 절차에서 비밀키 값 노출을 기계 검사한다(두 검사기의 비밀키 패턴은 단위 테스트로 동기화 고정). 다만 두 검사기 모두 알려진 접두 패턴 휴리스틱이므로, 통과가 키 부재를 보증하진 않는다 — 코드 인용에 키 값이 끼지 않았는지 사람이 함께 확인하기를 권한다.
