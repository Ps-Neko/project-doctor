...
## M. HTML/PDF 보안 검증기 갭 (2026-06-14, 검토 — 보류·트리거 조건부)

> 세션 시작 시 뜬 재평가(8.8→9.0)가 "다음 #1 = HTML/PDF 렌더러 코드화"로 지목. `report-formats.md`·백로그 코드 대조 후 검토 → **렌더러는 기각 유지, 검증기는 BL로 적고 트리거 조건부 보류** 결정.

- **[BL-31] HTML/PDF 보안 검증기 부재 — 보류(트리거 조건부 TDD)**. 갭은 실재이고 문서가 자인함: HTML/PDF는 코드가 아니라 모델이 `report-formats.md` 골격을 보고 직접 작성하고, 신뢰불가 값(분석 대상 파일명·코드 조각·소견) 이스케이프는 **프롬프트 규칙**(§2 절대 준수, 공격 예시 `</style><img onerror=…>.py` 명시)일 뿐 코드 강제가 아님. `.md`엔 형식검증기(`verify_report_format.py` FORM-11)가 완성본을 자동 검사하는데 **HTML/PDF는 검사 대상 밖**(§5가 "사람(Claude)이 직접 대조"라고 자인) — 이 **비대칭**이 핵심 갭.
  ...
  - **채택(B 빌드) 트리거**: ① HTML/PDF 내보내기를 외부/신뢰불가 제3자 레포에 실제로 쓰기 시작, ② 또는 외부 사용자 수요 신호(BL-14 충족). 그때 TDD 선행 — 정답지: 악성 파일명/소견 fixture로 미이스케이프 검출 + `<script>`·외부 리소스 거부 + `run_checks.py` 통합(HTML 있을 때만 작동 → 실사용 false-fail 0). north-star(병목=수요·코드 아님)와 정합 — 수요 신호 전 보류.
